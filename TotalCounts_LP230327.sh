# General comments:
# - Nowhere does the script mention the data is separated per chromosome, 
#   which is why it is required to iterate over files. 
#   In case the data is not separated in multiple (eg chromosome) chunks, does the script still work?
# - You use the concept 'count table', which is not a std concept. 
#   Either use a std concept (eg tab-separated file) and/or define your concept 
#   (eg count table; a tab-separated file containing ....)
# - you included an example CLI statement, which is good! 
#   But replace the actual filenames (with very long paths) with short pseudo-filenames
# - (minor:) Rather than writing intermediate counts to temp file,
#   could you collect them in a multi-dim bash array?
# There are also some inline comments below (marked "# COMMENT")
...
# COMMENT:
#   below, at 4 points the list of arguments is defined/used. There are 3 different combinations!
# Usage 
USAGE=
usage() {
...
# COMMENT; please document/describe this function (there is a lot of bash-magic which I don't understand, so it is far from self-explanatory)
function join_by {
  local d=${1-} f=${2-}
  if shift 2; then
    printf %s "$f" "${@/#/$d}"
...
# COMMENT: is next commentline still relevant?
